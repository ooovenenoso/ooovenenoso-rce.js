            ...
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    ...
                    if (!data?.data?.sendConsoleMessage?.ok) {
                        ...
                        ServerUtils_1.default.error(this._manager, `Failed To Send Command: ${detail}`, server);
                        CommandHandler_1.default.remove(CommandHandler_1.default.get(identifier, command));
                        resolve({
                            ok: false,
                            error: detail,
    ...
    async stop(identifier, force = false) {
        ...
        try {
            ...
            if (!response.ok || !data?.data?.stopService?.ok) {
                const detail = data?.errors?.[0]?.message ?? `HTTP ${response.status} ${response.statusText}`;
                ServerUtils_1.default.error(this._manager, `Failed To Stop Server: ${detail}`);
                return false;
            }
    ...
    async start(identifier) {
        ...
        try {
            ...
            if (!response.ok || !data?.data?.restartService?.cfgContext) {
                const detail = data?.errors?.[0]?.message ?? `HTTP ${response.status} ${response.statusText}`;
                ServerUtils_1.default.error(this._manager, `Failed To Start Server: ${detail}`);
                return false;
            }
